        ...
        $customer.select2({
            ...
            initSelection: function (element, callback) {
                $.ajax({
                    ...
                    url: site.base_url + "customers/getCustomer/" + slcustomer,
                    dataType: "json",
...
// Order level shipping and discount localStorage
    ...
    if (sltax2 = localStorage.getItem('sltax2')) {
        $('#sltax2').val(sltax2).trigger('change');
    }
...
// save and load the fields in and/or from localStorage
    ...
    $('#sldiscount').focus(function () {
        ...
        if (is_valid_discount(new_discount)) {
            ...
            addAlert(lang.unexpected_value, 'danger');
            return;
        }
    ...
    $(document).on('click', '#sellGiftCard', function (e) {
        if (count == 1) {
            ...
            if ($('#slwarehouse').val() && $('#slcustomer').val()) {
                ...
                $('#slwarehouse').prop("readonly", true);
            } else {
...
function loadItems() {
    ...
    if (localStorage.getItem('slitems')) {
        ...
        $.each(sortedItems, function () {
            ...
            tr_html += '<td class="text-right"><input class="balQty" name="balQty[]" type="hidden" value="' + item.row.quantity + '"><span class="text-right balQty" id="balQty_' + row_no + '">' + formatMoney(item.row.quantity) + '</span></td>';
...
function add_invoice_item(item) {
    ...
    if (count == 1) {
        slitems = {};
        if ($('#slwarehouse').val() && $('#slcustomer').val()) {
            $('#slcustomer').attr("readonly", true);
            $('#slwarehouse').prop("readonly", true);
        } else {
            addAlert(lang.select_above);
            item = null;
            return;
        }
    }
