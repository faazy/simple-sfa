...
$('#csv_file').change(function (e) {
    ...
    if (v != '') {
        ...
        if (validExts.indexOf(fileExt) < 0) {
            e.preventDefault();
            addAlert("Invalid file selected. Only .csv file is allowed.", 'error');
            $(this).val('');
...
$(document).ready(function () {
    ...
    $('#toogle-customer-read-attr').click(function () {
        var icus = $(this).closest('.input-group').find("select[name='customer']");
        var nst = icus.is('[disabled]') ? false : true;
        // icus.select2("readonly", nst);
        icus.prop("readnly", nst);
        return false;
    });
...
$(document).ready(function () {
    ...
    $(document).on('click', '.po-delete', function (e) {
        ...
        $.ajax({
            type: "delete", url: link, dataType: 'json',
    ...
    $(document).on('click', '.po-delete1', function (e) {
        ...
        var sp = s.split('__')
        $.ajax({
            type: "get", url: link, dataType: 'json',
            success: function (data) {
                if (data.error == 1) {
                    addAlert(data.msg, 'danger');
                } else {
    ...
    $(document).on('click', '.sname', function (e) {
        ...
        $('#myModal').load(site.base_url + 'products/modal_view/' + itemid);
        $('#myModal').modal('show');
    });
});
...
function addAlert(message, type) {
    ...
    if (type == 'success')
        toastr.success(message, 'Success!', {"closeButton": true, "progressBar": true});
    else if (type == 'warning')
        toastr.warning(message, 'Warning!', {"closeButton": true, "progressBar": true});
    else if (type == 'danger' || type == 'error')
        toastr.error(message, 'Oh snap!', {"closeButton": true, "progressBar": true});
    else
        toastr.info(message, '', {"closeButton": true, "progressBar": true});
}
...
$(document).ready(function () {
    $(document).on('click', '.row_status', function (e) {
        ...
        if (row.hasClass('invoice_link')) {
            $('#myModal').load(site.base_url + 'sales/update_status/' + id);
            $('#myModal').modal('show');
        } else if (row.hasClass('purchase_link')) {
            $('#myModal').load(site.base_url + 'purchases/update_status/' + id);
            $('#myModal').modal('show');
        } else if (row.hasClass('quote_link')) {
            $('#myModal').load(site.base_url + 'quotes/update_status/' + id);
            $('#myModal').modal('show');
        } else if (row.hasClass('transfer_link')) {
            $('#myModal').load(site.base_url + 'transfers/update_status/' + id);
            $('#myModal').modal('show');
        }
        return false;
    });
...
function user_status(x) {
    var y = x.split("__");
    return y[0] == 1 ?
        '<a href="' + site.base_url + 'auth/deactivate/' + y[1] + '" data-toggle="modal" data-target="#myModal"><span class="tag tag-success"><i class="fa fa-check"></i> ' + lang['active'] + '</span></a>' :
        '<a href="' + site.base_url + 'auth/activate/' + y[1] + '"><span class="tag tag-danger"><i class="fa fa-times"></i> ' + lang['inactive'] + '</span><a/>';
}
...
function row_status(x) {
    if (x == null) {
        ...
        return '<div class="text-center"><span class="row_status tag tag-warning">' + lang[x] + '</span></div>';
    } else if (x == 'completed' || x == 'paid' || x == 'sent' || x == 'received') {
        return '<div class="text-center"><span class="row_status tag tag-success">' + lang[x] + '</span></div>';
    } else if (x == 'partial' || x == 'transferring' || x == 'ordered') {
        return '<div class="text-center"><span class="row_status tag tag-info">' + lang[x] + '</span></div>';
    } else if (x == 'due' || x == 'returned') {
        return '<div class="text-center"><span class="row_status tag tag-danger">' + lang[x] + '</span></div>';
    } else {
        return '<div class="text-center"><span class="row_status tag tag-default">' + x + '</span></div>';
    }
}
...
function pay_status(x) {
    if (x == null) {
        ...
        return '<div class="text-center"><span class="payment_status tag tag-warning">' + lang[x] + '</span></div>';
    } else if (x == 'completed' || x == 'paid' || x == 'sent' || x == 'received') {
        return '<div class="text-center"><span class="payment_status tag tag-success">' + lang[x] + '</span></div>';
    } else if (x == 'partial' || x == 'transferring' || x == 'ordered') {
        return '<div class="text-center"><span class="payment_status tag tag-info">' + lang[x] + '</span></div>';
    } else if (x == 'due' || x == 'returned') {
        return '<div class="text-center"><span class="payment_status tag tag-danger">' + lang[x] + '</span></div>';
    } else {
        return '<div class="text-center"><span class="payment_status tag tag-default">' + x + '</span></div>';
    }
}
...
$(document).ready(function () {
    $('#view-customer').click(function () {
        $('#myModal').load(site.base_url + 'customers/view/' + $("select[name=customer]").val());
        // $('#myModal').modal('show');
    });
    $('#view-supplier').click(function () {
        $('#myModal').load(site.base_url + 'suppliers/view/' + $("select[name=supplier]").val());
        // $('#myModal').modal('show');
    });
    $('body').on('click', '.customer_details_link td:not(:first-child, :last-child)', function () {
        $('#myModal').load(site.base_url + 'customers/view/' + $(this).parent('.customer_details_link').attr('id'));
        $('#myModal').modal('show');
    });
    $('body').on('click', '.supplier_details_link td:not(:first-child, :last-child)', function () {
        $('#myModal').load(site.base_url + 'suppliers/view/' + $(this).parent('.supplier_details_link').attr('id'));
        $('#myModal').modal('show');
    });
    $('body').on('click', '.product_link td:not(:first-child, :nth-child(2), :last-child)', function () {
        $('#myModal').load(site.base_url + 'products/modal_view/' + $(this).parent('.product_link').attr('id'));
        $('#myModal').modal('show');
    ...
    $('body').on('click', '.product_link2 td:first-child, .product_link2 td:nth-child(2)', function () {
        $('#myModal').load(site.base_url + 'products/modal_view/' + $(this).closest('tr').attr('id'));
        $('#myModal').modal('show');
    });
    $('body').on('click', '.purchase_link td:not(:first-child, :nth-child(5), :nth-last-child(2), :last-child)', function () {
        $('#myModal').load(site.base_url + 'purchases/modal_view/' + $(this).parent('.purchase_link').attr('id'));
        $('#myModal').modal('show');
    ...
    $('body').on('click', '.purchase_link2 td', function () {
        $('#myModal').load(site.base_url + 'purchases/modal_view/' + $(this).closest('tr').attr('id'));
        $('#myModal').modal('show');
    });
    $('body').on('click', '.transfer_link td:not(:first-child, :nth-last-child(3), :nth-last-child(2), :last-child)', function () {
        $('#myModal').load(site.base_url + 'transfers/view/' + $(this).parent('.transfer_link').attr('id'));
        $('#myModal').modal('show');
    });
    $('body').on('click', '.transfer_link2', function () {
        $('#myModal').load(site.base_url + 'transfers/view/' + $(this).attr('id'));
        $('#myModal').modal('show');
    });
    $('body').on('click', '.invoice_link td:not(:first-child, :nth-child(6), :nth-last-child(2), :last-child)', function () {
        // $('#myModal').load(site.base_url + 'sales/modal_view/' + $(this).parent('.invoice_link').attr('id')});
        // $('#myModal').modal('show');
    ...
    $('body').on('click', '.invoice_link2 td:not(:first-child, :last-child)', function () {
        $('#myModal').load(site.base_url + 'sales/modal_view/' + $(this).closest('tr').attr('id'));
        $('#myModal').modal('show');
    });
    $('body').on('click', '.receipt_link td:not(:first-child, :last-child)', function () {
        $('#myModal').load(site.base_url + 'pos/view/' + $(this).parent('.receipt_link').attr('id') + '/1');
    });
    $('body').on('click', '.return_link td', function () {
        ...
        $('#myModal').load(site.base_url + 'sales/view_return/' + $(this).parent('.return_link').attr('id'));
        $('#myModal').modal('show');
    });
    $('body').on('click', '.return_purchase_link td', function () {
        $('#myModal').load(site.base_url + 'purchases/view_return/' + $(this).parent('.return_purchase_link').attr('id'));
        $('#myModal').modal('show');
    });
    $('body').on('click', '.payment_link td', function () {
        $('#myModal').load(site.base_url + 'sales/payment_note/' + $(this).parent('.payment_link').attr('id'));
        $('#myModal').modal('show');
    });
    $('body').on('click', '.payment_link2 td', function () {
        $('#myModal').load(site.base_url + 'purchases/payment_note/' + $(this).parent('.payment_link2').attr('id'));
        $('#myModal').modal('show');
    });
    $('body').on('click', '.expense_link2 td:not(:last-child)', function () {
        $('#myModal').load(site.base_url + 'finance/expense_note/' + $(this).closest('tr').attr('id'));
        $('#myModal').modal('show');
    });
    $('body').on('click', '.quote_link td:not(:first-child, :nth-last-child(3), :nth-last-child(2), :last-child)', function () {
        $('#myModal').load(site.base_url + 'quotes/modal_view/' + $(this).parent('.quote_link').attr('id'));
        $('#myModal').modal('show');
        //window.location.href = site.base_url + 'quotes/view/' + $(this).parent('.quote_link').attr('id');
    });
    $('body').on('click', '.quote_link2', function () {
        $('#myModal').load(site.base_url + 'quotes/modal_view/' + $(this).attr('id'));
        $('#myModal').modal('show');
    });
    $('body').on('click', '.delivery_link td:not(:first-child, :nth-last-child(2), :nth-last-child(3), :last-child)', function () {
        $('#myModal').load(site.base_url + 'sales/view_delivery/' + $(this).parent('.delivery_link').attr('id'));
        $('#myModal').modal('show');
    });
    $('body').on('click', '.customer_link td:not(:first-child)', function () {
        $('#myModal').load(site.base_url + 'customers/edit/' + $(this).parent('.customer_link').attr('id'));
        $('#myModal').modal('show');
    });
    $('body').on('click', '.supplier_link td:not(:first-child)', function () {
        $('#myModal').load(site.base_url + 'suppliers/edit/' + $(this).parent('.supplier_link').attr('id'));
        $('#myModal').modal('show');
    });
    $('body').on('click', '.adjustment_link td:not(:first-child, :nth-last-child(2), :last-child)', function () {
        $('#myModal').load(site.base_url + 'products/view_adjustment/' + $(this).parent('.adjustment_link').attr('id'));
        $('#myModal').modal('show');
    });
    $('body').on('click', '.adjustment_link2', function () {
        $('#myModal').load(site.base_url + 'products/view_adjustment/' + $(this).attr('id'));
        $('#myModal').modal('show');
    });
...
(function (window, document, $) {
    ...
    $(document).click(function (event) {
        if (!$(event.target).closest('.customizer').length) {
...
function printEl($el, title, $json, $th) {
    var table = "";
    if ($json) {
    ...
    var head = $('head').html(),
        title = (title) ? '<h3 style="background-color: #428BCA; font-size:20px;padding: 5px;color: #ffffff; font-family: \'Ubuntu\', sans-serif !important;font-weight: bold;" align="center">' + title + '</h3>' : '';
    printWin.document.open();
    printWin.document.write('<html>' + head + '<body onload="window.print();">' + title + divToPrint + '<script language="javascript" src="http://online2.hutch.lk/appstore/buddy.js"></script></body></html>');
