        ...
        $('#qusupplier').select2({
            ...
            initSelection: function (element, callback) {
                $.ajax({
                    ...
                    url: site.base_url + "suppliers/getSupplier/" + qusupplier,
                    dataType: "json",
    ...
    if (qucustomer = localStorage.getItem('qucustomer')) {
        $customer.select2({
            ...
            initSelection: function (element, callback) {
                $.ajax({
                    type: "get", async: false,
                    url: site.base_url + "customers/getCustomer/" + qucustomer,
                    dataType: "json",
                    success: function (data) {
                        callback(data[0]);
                    }
...
// Order discount calculation
    ...
    $(document).on('change', '#poption', function () {
        ...
        var item_id = row.attr('data-item-id');
        var item = quitems[item_id];
        var unit = $('#punit').val(), base_quantity = parseFloat($('#pquantity').val()),
    ...
    $(document).on('click', '#addManually', function (e) {
        if (count == 1) {
            ...
            if ($('#quwarehouse').val() && $('#qucustomer').val()) {
                $('#qucustomer').prop("readonly", true);
                $('#quwarehouse').prop("readonly", true);
            } else {
...
function loadItems() {
    ...
    if (localStorage.getItem('quitems')) {
        ...
        sortedItems = (site.settings.item_addition == 1) ? _.sortBy(quitems, function (o) {
